...
\begin{frontmatter}
\begin{abstract}
Following the exponential growth in the number of readily available documents and texts in recent years, the need for efficient methods of classification has become increasingly important. In many cases, the available data for machine learning is abundant, but the corresponding classes, also known as labels, are expensive to obtain. Active Learning is a subfield of machine learning that attempts to combat this issue by selecting instances from an unlabelled pool such that they are annotated by some information source. This selection is performed using a \textit{query strategy}, which decides which instances are most conducive to the learning process and should thus be annotated. One such query strategy is the Core-Set approach (\cite{DBLP:conf/iclr/SenerS18}), a diversity-based query strategy that operates using the distances between instances and attempts to select sample such that they best cover the rest of the dataset. Although this approach is well-established within the AL field, it has shown mixed results within the field of text classification. In this thesis, I aim to investigate the performance of Core-Set when applied to the task of text classification, as well as propose three approaches (dimensionality reduction-based, uncertainty-based, class balance-based) that iterate upon the original strategy. The experiments conducted within this thesis show that within the domain of text classification, Core-Set has mixed results when compared to well-known baseline approaches. Additionally, this thesis demonstrates minor performance increases when combined with two dimensionality reduction techniques; however, the uncertainty- and class balance-based approaches do not lead to significant improvements.
\end{abstract}
\end{frontmatter}
